...
void test_search() {
  ...
  const int key = 8;
  Array a;
//  test(a.end(), lb_search, a.begin(), a.end(), key); // Degerate (empty array)
...
//  test(found2, lb_search, e.begin(), e.end(), key);
  ...
  test(-1, bin_search, Array({1, 2, 3, 4, 5, 6, 7}), key);
    ...
    test(0, bin_search, Array({key, key + 1}), key); // Trivial2 (2 elements array)
    test(1, bin_search, Array({key - 1, key}), key); // Trivial2 (2 elements array)
